...
- PR 评论已记录：当前 OAuth token 缺少 GitHub contents 写入 scope，连接的 GitHub App 也无写入仓库内容权限，因此 `0aae184` 不能推送；这不是代码运行卡死。
...
- 不通过修改 OAuth token、替换凭证或绕开 GitHub 权限来推送。
...
4. **RTP-4 外部决策**：等待有 contents 写入权限的用户/凭证推送 `0aae184`，随后由 GitHub 触发该工作流；Ready、审阅和合并仍由用户/仓库流程决定。
...
1. GitHub OAuth token 或连接 App 仍没有仓库 contents 写入权限；
