...
>
> 完成状态：SEC2-001～009 已于 2026-07-01 依序实现；当前生产默认 V2，V1 仅作临时回退。
...
完成证据：水平/斜切正方体与 18 方块阶梯确定性帧序列通过；进入、穿过、离开无非法空帧或
残留；BufferGeometry 身份固定、扩容受控、相同帧跳过 GPU 写入。
...
完成：提交 `7e9afe2`，1280×720 单视口工作台通过；窄屏恢复自然滚动。

### UX2-003 feat: 建立视角与切面拖拽模式

必须在 SEC2-009 后进行。先写交互状态机，不直接把 OrbitControls 与切面拖拽监听混在一起。

完成：提交 `9eb075a`；orbit/plane 互斥状态机、单 pointer capture、三点锁定保护和真实手势通过。
...
| 顺序 | 任务 | 难度 | 说明 |
|---:|---|---:|---|
| 1 | CUT-FIX-007 参考视频体验录制 | 中 | 验证肉眼连续性、构图、控件可达性 |
| 2 | 关闭旧 CUT-FIX-006 阻塞项 | 低 | 只整理 V2 证据，不重写算法 |
| 3 | COM-007 组合体恢复验收 | 中高 | 重点验证分层多 Mesh 接缝与多轮廓 |
| 4 | 5×5×5 分层搭建器 | 中高 | 复用现有积木数据和 V2，禁止另写截面算法 |

算法已经按“切片、归一化、邻接闭环、拓扑、三角化、稳定渲染”拆成独立模块。后续 Agent
不需要重新发明数学，只需按模块契约补样例或接 UI。最危险的扩展是未布尔合并的重叠壳体和
复杂 CSG；它们必须另建黄金样例与任务，不能在 UX 工作中顺手修改。
