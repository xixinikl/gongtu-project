...
两题动态讲解未通过前，禁止开始通用图片自动建模。AI 以后只能生成待确认草稿，不能生成正式
答案或绕过人工确认。
...
| 后续任务 | 难度 | 原因与边界 |
|---|---:|---|
| CASE-001 圆锥方体黄金题 | 中 | 必须人工固定题图、选项、答案、关键切面和排除理由 |
| CASE-002 棱锥圆柱黄金题 | 中 | 必须固定圆柱与棱锥侧面约束，不能由 AI 猜答案 |
| LESSON-001～007 动态讲解闭环 | 中高 | 新增学生入口，但只复用现有模型与截面模块 |
| AUTHOR-001～002 手工录题 | 中高 | 没有 AI 时也必须能完整制作题目 |
| VISION-001～007 图片辅助录题 | 高 | 只允许本地开源模型和人工确认后的草稿 |
| 任意重叠壳体布尔并集截面 | 很高 | 当前重叠封闭壳体会明确报 topology error；若要支持，应先做可靠并集表面 |
| 洞、内腔与复杂 CSG 回归 | 高 | 拓扑已支持孔洞，但真实 CSG 网格质量和共面碎片需要独立黄金样例 |

唯一下一项是 CASE-001：建立“倒圆锥 + 方体”参考题黄金答案。不要直接开始 AI 图片识别。
