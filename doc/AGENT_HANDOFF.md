...
该文档优先级高于旧的 `LESSON-013` 文字修补。用户已明确否决“只用文字说明 B 六边形”的方案；
后续必须按“候选驱动验证”返工：先把切面摆到尽量像候选，再在真实截面上显示多出来、
缺掉、带出来或接触处不一致的部分。
...
- 但 `LESSON-013` 关于 B 六边形的修补只是文字层面，已被用户否决。不能继续沿用为最终方案。
  正确返工见 `LESSON-014R`：B 选项必须有接近六边形的真实切面尝试，并显示方体/倒圆锥/接触结构
  被带出来。
