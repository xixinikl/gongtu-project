...
- [ ] ◐ SEC-1B 修正统一登录页的管理员初始化说明并建立前端合同回归。
- [ ] ○ SEC-2 登录与注册防爆破：按来源和账号限流、输入长度上限、可信代理显式配置。
- [ ] ○ SEC-3 管理员高风险操作审计：授权、VIP、策略与删除操作可追踪且不记录敏感明文。
- [ ] ○ SEC-4 SQLite 自动备份、完整性验证、保留策略与恢复演练工具。
- [ ] ○ SEC-5 生产就绪检查：密钥、数据库权限、备份新鲜度、运行模式和部署前停止条件。
