...
- [ ] ◐ GT-P6-5E 申论批改幂等后端：按 `users.id + Idempotency-Key` 保存请求哈希与状态；同键同载荷回放同一结果、不同载荷409、处理中409、失败安全回放，新键才允许有意重做。
- [ ] ○ GT-P6-5F 申论批改幂等界面：每次有意提交生成并复用请求键，网络重试不得重复写记录；安全错误对象显示人类可读文案。
