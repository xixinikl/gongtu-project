...
- Phase 6：从 Phase 5 精确提交 `4afaf90` 建立独立工作树；GT-P6-1—6 已有直接证据，尚待建立 PR #6 和总装回归。
...
- GT-P6-7 审计进行中：已确认 Phase 1→6 的提交链连续；PR #15—#19 都是 open 且未合并（#17—#19 为 Draft），因此总装只能建立为独立待审分支，不能声称已合并或已获得审阅。9 个 worktree 的只读盘点与三分类见 `doc/PHASE6_COMPLETION_AUDIT.md`。
...
1. 完成 GT-P6-7—8：建立 Phase 6 Draft PR 与干净总装分支，在总装分支重跑完整回归和浏览器主路径；外部审阅/合并保持明确待办。
