...
- 当前 GT-P6-5E：为申论正式批改增加用户级幂等合同，重复请求不得重复调用Provider或重复写历史、问题、证据与任务。
...
1. 完成 GT-P6-5E/5F 申论批改幂等后端与正式界面合同。
2. 完成正式外壳移动端导航、键盘、焦点与减少动效整改。
3. 依次执行六模块真实主路径、身份持久化、A/B 隔离、失败路径和响应式门禁。
4. 建立 Phase 6 PR 后执行六阶段干净总装与剩余改动三分类。
